<template>
    <div class="sider" style="overflow:hidden;">
        ...
        <div class="nav">
            ...
            <!-- 展示文件夹列表 -->
            <div class="book" v-for="(book,index) in allNotebooks" :key="`book${book.id}`" v-if="allNotebooks&&allNotebooks.length&&!retractBooks" :class="{active:selectedBook&&book.id===selectedBook.id}" @click="onClickBook($event,index,book)" @click.right="onClickBook($event,index,book)">
